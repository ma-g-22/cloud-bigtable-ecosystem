...
class BigtableCqlSessionNodeStateListener extends NodeStateListenerBase {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(BigtableCqlSessionNodeStateListener.class);
  ...
  public void onDown(Node node) {
    ...
    if (currentUp <= 0) {
      LOGGER.warn("All nodes are down. Closing the CqlSession.");
